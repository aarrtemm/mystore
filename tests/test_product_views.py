...
class ProductListViewTest(TestCase):
    def setUp(self):
        ...
        Product.objects.create(
            name="TestProduct", price=10, quantity=5, gender=gender, image=IMAGE_FILE
        )
    ...
    def test_product_list_view(self):
        ...
        self.assertQuerysetEqual(
            response.context["products"], ["TestProduct"], transform=lambda x: x.name
        )
        self.assertQuerysetEqual(
            response.context["categories"], ["TestCategory"], transform=lambda x: x.name
        )
        self.assertQuerysetEqual(
            response.context["genders"], ["TestGender"], transform=lambda x: x.name
        )
...
class ProductDeleteViewTest(TestCase):
    def setUp(self):
        gender = Gender.objects.create(name="TestGender")
        self.product = Product.objects.create(
            name="TestProduct", price=10, quantity=5, gender=gender
        )
...
class BasketRemoveViewTest(TestCase):
    def setUp(self):
        ...
        self.url = reverse(
            "products:basket_remove", kwargs={"basket_id": self.basket.id}
        )
